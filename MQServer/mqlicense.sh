...
UNAME_FLAG="-i | sed s/unknown/\$(uname -m)/"
#############################################################################
#
#   <copyright
...
#  These values need to be in sync with preinstall.sh present in install/unix/linux_2
#######################################################################################################
typeset -i MIN_SLES_VERSION=15
typeset -i MIN_SLES_RELEASE=4
typeset -i MIN_RH_VERSION=8
typeset -i MIN_RH_RELEASE=8
typeset -i MIN_UBUNTU_VERSION=22
...
# Set JRE location
# There could be multiple "bin" directories under "lap", but we'll assume/hope that the first will contain a "java" executable
LAPJRE="${LAPJRE:-$(find $PROGPATH/lap -type d -name bin | head -n 1)/java}"
if [ ! -x "${LAPJRE}" ]; then
